...
- **WHEN** 住所から緯度経度を取得する
- **THEN** 公開地図用の緯度経度は Google Geocoding API で統一生成する
- **AND** 既存CSVに緯度経度がある場合も、公開用座標として混在させず同じGoogleジオコーディング処理で作り直す
- **AND** GSI Japan Address Search API は比較・監査用に限定し、公開用座標生成の通常経路に含めない
