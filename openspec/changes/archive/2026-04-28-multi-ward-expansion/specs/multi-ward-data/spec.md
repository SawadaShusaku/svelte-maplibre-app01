...
アプリは選択された区のGeoJSONファイルを動的にロードしなければならない（SHALL）。新しい区のGeoJSONファイルを `src/lib/data/{prefecture}/{city}.geojson` に追加し `registry.ts` に1行追記するだけで地図に表示できる。
...
### Requirement: 区レジストリ
`src/lib/registry.ts` に利用可能な区のメタ情報（prefecture・city・cityLabel・prefectureLabel）を定義しなければならない（SHALL）。
